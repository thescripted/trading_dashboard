...
const Header = ({ queryResultCallBack }) => {
  ...
  return (
    <div className="flex flex-row content-center p-4 md:py-4 md:px-8 justify-between bg-blue-600 space-x-2">
      <a href="/" className="pointer self-center">
        <h2 className="text-white text-l hover:text-white self-center sm:text-xl">
          Trading Analytics
        </h2>
      </a>
      <span className="relative w-full flex space-y-16">
        <input
          className="bg-gray-300 rounded w-full p-2 self-center h-10"
          placeholder="Search"
        ...
        {state.isDropdown && (
          ...
          >
            {state.queryStockResults.length === 0 ? (
              <h1 className="text-l text-gray-500 py-4 sm:text-xl">
